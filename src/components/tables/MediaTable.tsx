...
const MediaTable: React.FC = () => {
  ...
  const columns = useMemo(
    () => [
      ...
      columnHelper.accessor("createdAt", {
        header: "Created At",
        cell: (info) => (
          <span className="text-gray-400">{ddmmyyyy(info.getValue())}</span>
        ),
      }),
        ...
        cell: ({ row }) => {
          ...
          return (
            <div className="flex items-center gap-2">
              ...
              {/* Edit */}
              <button
                onClick={() => router.push(`/admin/media/edit/${image.id}`)}
                className="p-2 rounded cursor-pointer hover:bg-gray-700 transition"
              >
  ...
  if (loading)
    return <CommonTableSkeleton columns={4} rows={10} showSearch={true} />;
