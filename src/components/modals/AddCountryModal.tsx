...
const AddCountryModal: React.FC<AddCountryModalProps> = ({ isOpen, onClose, onSuccess }) => {
    ...
    const handleSubmit = async (values: CountryFormValues, { setSubmitting, resetForm }: any) => {
        // console.log("--- response in the submitting the new country ---", values);
        try {

            // eslint-disable-next-line @typescript-eslint/no-explicit-any
            const response: any = await dispatch(createCountry(values));
            console.log("--- response in the submitting the new country ---", response);
    ...
    return (
        <div className="fixed inset-0 z-50 overflow-y-auto">
            ...
            <div className="flex min-h-full items-center justify-center p-4">
                <div className="relative transform overflow-hidden rounded-lg bg-[#1a1a1a] shadow-xl transition-all w-full max-w-lg">
                    {/* Header */}
