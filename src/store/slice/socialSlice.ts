...
export const deleteSocial = createAsyncThunk<{ id: string }, string>(
  ...
  async (id) => {
    ...
    console.log("socials[index].id", response);
    return { id };
  },
);
...
const socialSlice = createSlice({
  ...
  extraReducers: (builder) => {
    ...
    builder.addCase(
      getSocials.fulfilled,
      (state, action: PayloadAction<Social[]>) => {
        state.loading = false;
