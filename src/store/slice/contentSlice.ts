...
interface ContentState {
  contents: Record<string, Content>; // key = page
  loading: boolean;
  error: string | null;
}
...
export const fetchAllContent = createAsyncThunk<Content[]>(
  "content/fetchAllContent",
  async (_, thunkAPI) => {
    try {
      const res = await api<{ contents: Content[] }>({
        url: "/admin/content",
        method: "GET",
      });
...
export const fetchContent = createAsyncThunk<Content, string>(
...
export const saveContentThunk = createAsyncThunk<Content, Content>(
  "content/saveContent",
  async ({ page, html, title }, thunkAPI) => {
    try {
      const res = await api<Content, Content>({
        url: "/admin/content",
        method: "POST",
...
const contentSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(fetchAllContent.fulfilled, (state, action: PayloadAction<Content[]>) => {
        state.loading = false;
        state.contents = {};
        action.payload.forEach((c) => {
          state.contents[c.page] = c;
        });
      })
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...
      .addCase(fetchContent.fulfilled, (state, action: PayloadAction<Content>) => {
        state.loading = false;
        state.contents[action.payload.page] = action.payload;
      })
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      ...
      .addCase(saveContentThunk.fulfilled, (state, action: PayloadAction<Content>) => {
        state.loading = false;
        state.contents[action.payload.page] = action.payload;
      })
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
